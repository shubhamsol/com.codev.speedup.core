...
namespace deVoid.UIFramework {
    ...
    public class PanelUILayer : AUILayer<IPanelController> {
        ...
        public override void ShowScreen(IPanelController screen) => screen.Show();

        public override void ShowScreen<TProps>(IPanelController screen, TProps properties) => screen.Show(properties);

        public override void HideScreen(IPanelController screen, bool animate = true) => screen.Hide(animate);

        public bool IsPanelVisible(string panelId) {
            if (registeredScreens.TryGetValue(panelId, out var panel)) {
