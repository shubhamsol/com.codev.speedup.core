...
namespace deVoid.UIFramework
{
    ...
    public class WindowUILayer : AUILayer<IWindowController>
    {
        ...
        public override void ShowScreen(IWindowController screen) => ShowScreen<IWindowProperties>(screen, null);
        ...
        private void DoShow(IWindowController screen, IWindowProperties properties) => DoShow(new WindowHistoryEntry(screen, properties));

        private void DoShow(WindowHistoryEntry windowEntry) {
            if (CurrentWindow == windowEntry.Screen) {
                Debug.LogWarning($"[WindowUILayer] The requested WindowId ({CurrentWindow.ScreenId}) is already open! This will add a duplicate to the history and might cause inconsistent behaviour.");
            }
        ...
        private void OnCloseRequestedByWindow(IUIScreenController screen) => HideScreen(screen as IWindowController);
